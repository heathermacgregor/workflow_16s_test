            ...
            try:
                ...
                if status_code == 503:
                    ...
                    if attempt < max_retries:
                        ...
                        logger.warning(f"SoilGrids API returned 503 (Service Unavailable). Retrying in {wait_time}s (attempt {attempt + 1}/{max_retries + 1})...")
                ...
                if attempt < max_retries:
                    wait_time = backoff_base * (backoff_multiplier ** attempt)
                    logger.warning(f"SoilGrids API connection error (possibly too many 503s). Retrying in {wait_time}s (attempt {attempt + 1}/{max_retries + 1})... Error: {str(e)}")
                    time.sleep(wait_time)
                    continue
                else:
                ...
                if attempt < max_retries and 'timeout' in str(e).lower():
                    wait_time = backoff_base * (backoff_multiplier ** attempt)
                    logger.warning(f"SoilGrids API timeout. Retrying in {wait_time}s (attempt {attempt + 1}/{max_retries + 1})...")
                    time.sleep(wait_time)
                    continue
                else:
