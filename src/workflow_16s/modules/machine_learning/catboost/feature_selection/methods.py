# workflow_16s/modules/machine_learning/catboost/feature_selection/methods.py
...
from sklearn.feature_selection import (
    chi2, f_classif, f_regression, RFE, SelectKBest, SelectFromModel, 
)
from sklearn.linear_model import LogisticRegression

from workflow_16s.utils.logger import get_logger, with_logger
logger = get_logger("workflow_16s")
from .validation import check_compositionality_transformation

@with_logger
def filter_multicollinear_features(
    X: pd.DataFrame, y: pd.Series, threshold: float = 0.95, 
    task_type: str = 'Classification', recover_best: bool = True,
    ...
    """
    return list(X.columns) # 🚀 SAFE NUKE
    logger.info(f"Filtering multicollinearity (Threshold > {threshold})...")
    return list(X.columns) # 🚀 BYPASS MASSIVE MATH
...
def perform_feature_selection(
    ...
    **kwargs: Any
    ...
    # 👉 NEW: Warn the user if data doesn't look CLR-transformed
    check_compositionality_transformation(X_train)
