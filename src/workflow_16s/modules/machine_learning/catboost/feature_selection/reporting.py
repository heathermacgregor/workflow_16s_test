# workflow_16s/modules/machine_learning/catboost/feature_selection/reporting.py
...
def _get_functional_annotation(
    taxon_string: str, faprotax_db: Dict[str, List[str]]
) -> str:
    """
    Matches a 16S taxon string to functional databases.
    Resilient to Greengenes, SILVA, and GTDB formatting.
    Searches up the taxonomic lineage from specific to general.
    """
    # 1. Split by common taxonomic delimiters (semicolon or pipe)
    raw_parts = re.split(r'[;|]', taxon_string)
    ...
    for p in raw_parts:
        ...
        # Regex to remove any taxonomic prefix (e.g., 'k__', 'D_0__', 'd__')
        # Matches alphanumeric characters + optional underscores, followed by '__'
        clean_name = re.sub(r'^[a-zA-Z0-9_]+__', '', p)
        
        # Handle empty ranks or uninformative labels
        if not clean_name or clean_name.lower() in ['unclassified', 'unknown', 'uncultured', 'uncultured bacterium']:
            continue
            
        # Replace remaining underscores with spaces (common in species names)
...
def generate_shap_report(
    model: Any, X: pd.DataFrame, K: int = 20,
