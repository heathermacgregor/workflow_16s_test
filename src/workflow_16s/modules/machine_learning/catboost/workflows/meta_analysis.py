# workflow_16s/modules/machine_learning/catboost/workflows/meta_analysis.py
...
from workflow_16s.visualization.machine_learning import generate_study_overlap_matrix


@with_logger
def perform_meta_analysis(
    X_df: pd.DataFrame, y_series: pd.Series, batches: np.ndarray, 
    output_dir: Path, n_top_features: int = 20
) -> Dict[str, Any]:
    """
    ...
    logger.info(f"ℹConsensus established from {valid_studies} eligible studies.")
...
def apply_meta_consensus_weighting(
    global_importance_df: pd.DataFrame, meta_results: Dict[str, Any],
