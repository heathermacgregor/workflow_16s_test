# workflow_16s/modules/machine_learning/catboost/workflows/standard_analysis.py
...
def run_machine_learning_analysis(
    adata: ad.AnnData, plot_dir_ml: Path, level: str = 'Genus', 
    min_samples_per_group: int = 10, max_classes: int = 10, 
    priority_targets: Optional[List[str]] = None, strict_targets: bool = False, 
    validate_overfitting: bool = True, quick_validation: bool = False,
    batch_config: Optional[Dict[str, Any]] = None, ml_config: Optional[MLConfig] = None, 
            ...
            if batch_enabled and batch_covariates_df is not None:
                ...
                try:
                    ...
                    if validate_overfitting:
                        # 💡 FIX: Removed invalid 'n_permutations' and 'quick_mode' arguments.
                        # We now translate 'quick_validation' into fewer CV splits to save time.
                        run_comprehensive_validation(
                            model=model, 
                            X=X, 
                            y=y, 
                            output_dir=plot_dir_ml / algo,
                            target_name=target_col, 
                            task_type=task_type.capitalize(),
                            n_cv_splits=3 if quick_validation else 5,
                            groups=None # Standard analysis doesn't assume group-aware splits natively
                        )
                        
                except Exception as e: 
                    logger.error(f"Standard ML failed for {algo}: {e}")
                    import traceback
