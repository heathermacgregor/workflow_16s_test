...
def _get_dense_matrix(X) -> np.ndarray:
    """
    Safely convert AnnData matrix to dense numpy array.
    
    Handles sparse matrices (CSR, CSC), dense arrays, H5Array, etc.
    """
    if sp.issparse(X):
        return X.toarray()
        ...
        # Try direct conversion for array-like objects
        return np.asarray(X)
...
HUMAN_CONTAMINANTS = {
    'skin': [
        'Propionibacterium', 'Cutibacterium', 'Staphylococcus',
...
def _frequency_based_detection(adata: ad.AnnData) -> pd.Series:
    ...
    for i in range(X.shape[1]):
        ...
        try:
            result = spearmanr(total_reads, feature_abundance)
            # Handle both tuple and SignificanceResult return types
            if hasattr(result, 'correlation'):  # SignificanceResult object
                corr = result.correlation # type: ignore
                pval = result.pvalue # type: ignore
            else:  # Tuple return (older scipy versions)
                corr, pval = result
            
            # Negative correlation suggests contamination
            if corr < 0 and pval < 0.05: # type: ignore
                # Score based on strength of negative correlation
                scores.iloc[i] = min(1.0, abs(corr)) # type: ignore
...
def detect_cross_sample_contamination(
    adata: ad.AnnData,
