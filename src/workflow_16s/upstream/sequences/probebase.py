...
# Centralize URLs for easier maintenance
...
def import_and_save_database(
    ...
    tsv_path: Path = OUTPUT_TSV_PATH,
    db_path: Path = DB_PATH
):
    """
    Orchestrates the entire process of downloading primer data and saving it
    to a local SQLite database if it doesn't already exist.

    Args:
        csv_path (Path): Path to save the initial downloaded CSV.
        tsv_path (Path): Path to save the intermediate scraped TSV data.
        db_path (Path): Path for the final SQLite database.
    """
    if db_path.exists():
    ...
    with requests.Session() as session:
        # Set a browser-like User-Agent to prevent being blocked
        session.headers.update({
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36"
        })

        if not download_probebase_csv(session, csv_path):
            logger.critical("Failed to download initial data. Aborting database creation.")
            return

        process_and_save_primer_data(session, csv_path, tsv_path)

    # Convert the TSV to an optimized SQLite database
    if tsv_path.exists():
        logger.info("\n--- Converting TSV to SQLite Database ---")
        create_and_populate_db(tsv_path, db_path)
    else:
        logger.error("Could not create database because the intermediate TSV file was not created.")
