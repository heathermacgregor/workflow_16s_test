...
from workflow_16s.visualization.fastqc import FastQCPlots
...
class SeqKit:
    ...
    def analyze_samples(self, samples: Dict[str, 
                                            List[Union[str, Path]]]) -> Dict[str, Any]:
        ...
        with get_progress_bar() as progress:
            task = progress.add_task("Running SeqKit...", total=len(file_list))
            ...
            elapsed = progress.tasks[task].elapsed or 0.0
    ...
    def _process_file(self, sample: str, fpath: Path) -> Tuple:
        ...
        stats_output = self._run_seqkit(["seqkit", "stats", "-T", "--all", str(fpath)])
        stats = self._parse_stats(stats_output)
        
        # Get length distribution
        lengths_output = self._run_seqkit(["seqkit", "fx2tab", "-n", "-l", str(fpath)])
        length_counts = Counter(int(line.split("\t")[1]) 
                                for line in lengths_output.strip().split("\n"))
        
        return (
            sample, 
            {
                "total_seqs": stats["num_seqs"], "total_bases": stats["sum_len"],
                "min_length": stats["min_len"], "max_length": stats["max_len"],
                "length_counts": length_counts, "file": fpath.name
            }
        )
    ...
    def _init_aggregators(self, samples: Dict) -> tuple:
        agg_stats = defaultdict(
            lambda: {
                "total_seqs": 0, "total_bases": 0, "min_length": float('inf'),
                "max_length": 0, "length_counts": Counter(), "files": []
            }
        )
        ...
        overall = {
            ...
            "total_seqs": 0, "total_bases": 0,
            "global_min": float('inf'), "global_max": 0,
    ...
    def _build_sample_metrics(self, data: dict) -> dict:
        ...
        return {"total_sequences": data["total_seqs"], "total_bases": data["total_bases"],
                "avg_length": self._calculate_avg(data["total_bases"], data["total_seqs"]),
                "min_length": data["min_length"], "max_length": data["max_length"],
                "file_count": len(data["files"]), "files": data["files"],
    ...
    def _build_overall_metrics(self, data: dict) -> dict:
        ...
        return {"total_samples": data["total_samples"], "total_files": data["total_files"],
                "total_sequences": data["total_seqs"], "total_bases": data["total_bases"],
                "avg_length": self._calculate_avg(data["total_bases"], data["total_seqs"]),
                "min_length": data["global_min"], "max_length": data["global_max"],
                "most_common_lengths": data["length_distribution"].most_common(10)}

    def _build_timing_metrics(self, elapsed: float, files: int, sequences: int) -> Dict:
        """Construct processing time metrics dictionary."""
        return {"execution_seconds": elapsed,
    ...
    def _finalize_results(self, agg_stats: Dict, overall: Dict, elapsed: float) -> Dict:
        ...
        return {"samples": {sample: self._build_sample_metrics(data) 
                            for sample, data in agg_stats.items()},
                "overall": self._build_overall_metrics(overall),
                "proc_time": self._build_timing_metrics(elapsed, overall["total_files"], 
                                                        overall["total_seqs"])}
...
class CutAdapt:
    ...
    Attributes:
        ...
        trimmed_fastq_dir: Directory to save trimmed FASTQ files.
        primer_fwd:        Forward primer sequence.
        primer_rev:        Reverse primer sequence.
        start_trim:        Number of bases to trim from start.
        end_trim:          Number of bases to trim from end.
        start_q_cutoff:    Quality cutoff for start.
        end_q_cutoff:      Quality cutoff for end.
        min_seq_length:    Minimum sequence length to retain.
        cores:             Number of CPU cores to use.
        rerun:             Whether to rerun if output exists.
        region:            Target region (e.g. V4).
    """
    def __init__(
        self, fastq_dir: Union[str, Path], trimmed_fastq_dir: Union[str, Path],
        primer_fwd: str, primer_rev: str, start_trim: int = DEFAULT_START_TRIM,
        end_trim: int = DEFAULT_END_TRIM, start_q_cutoff: int = DEFAULT_START_Q_CUTOFF,
        end_q_cutoff: int = DEFAULT_END_Q_CUTOFF, min_seq_length: int = DEFAULT_MIN_SEQ_LENGTH,
        cores: int = DEFAULT_N_CORES_CUTADAPT, rerun: bool = DEFAULT_RERUN_CUTADAPT,
    ...
    def run_cutadapt(parameters: Dict) -> Union[str, None]:
        ...
        try:
            command = [
                "conda", "run", "-n", "workflow_16s", "cutadapt", 
                "--report=minimal"
            ]
            if parameters["cores"] is not None:
                command.extend(["--cores", str(parameters["cores"])])
            if (parameters["start_q_cutoff"] is not None
    ...
    @staticmethod
    def process_sample(params: Dict) -> Tuple[str, Union[str, None, Exception]]:
        """Processes a single sample with error handling."""
        try:
            result = CutAdapt.run_cutadapt(params)
    ...
    def parse_metrics_to_dataframe(data_tuples: List[Tuple[str, str]]) -> pd.DataFrame:
        ...
        for sample_id, metrics_str in data_tuples:
            ...
            if len(headers) != len(values): continue
            row = {'ID': sample_id}
    ...
    def run(self, fastq_paths: Dict[str, List[str]]) -> Tuple[Dict[str, List[str]], pd.DataFrame, Dict[str, Any]]:
        ...
        for sample in samples:
            ...
            all_params.append({
                ...
                'primer_fwd': self.primer_fwd, 'primer_rev': self.primer_rev,
                'start_trim': self.start_trim, 'end_trim': self.end_trim,
                'start_q_cutoff': self.start_q_cutoff, 'end_q_cutoff': self.end_q_cutoff,
                'min_seq_length': self.min_seq_length, 'cores': 4, 'rerun': self.rerun
            })
        results = []
        with get_progress_bar() as prog:
            task = prog.add_task("Running CutAdapt...", total=len(samples))
            ...
            elapsed = prog.tasks[task].elapsed or 0.0
            proc_time = {
...
class BasicStats:
    """A class to calculate basic sequencing statistics from FASTQ.GZ files.
    ...
    def calculate_statistics(
        self, samples: Dict[str, List[Union[str, Path]]]) -> Dict[str, Any]:
        ...
        with get_progress_bar() as prog:
            task = prog.add_task("Analyzing runs...", total=len(file_list))
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                futures = {
    ...
    def _flatten_samples(self, samples: Dict[str, List[Union[str, Path]]]) -> List:
        ...
        return [(sample, fpath) for sample, paths in samples.items()
                for fpath in paths]

    def _initialize_structures(self, samples):
        """Create initial data structures for aggregation."""
        sample_agg = defaultdict(
            lambda: {
                "total_sequences": 0, "total_bases": 0, "total_gc": 0, 
                "min_length": float("inf"), "max_length": 0
            }
        )

        overall_stats = {
            "total_samples": len(samples), "total_sequences": 0,
            "total_bases": 0, "avg_length": None, "min_length": None,
            "max_length": None, "gc_percent": None
        }
    ...
    def _finalize_stats(self, sample_agg, overall_stats, elapsed):
        ...
        for sample, agg in sample_agg.items():
            ...
            stats = {
                "total_sequences": total_seq, "total_bases": agg["total_bases"],
        ...
        if overall_stats["total_sequences"] > 0:
            overall_stats.update({
                ...
                "gc_percent": (sum(a["total_gc"] for a in sample_agg.values()) / overall_stats["total_bases"] * 100),
    ...
    def process_file(sample: str, file_path: Union[str, Path]) -> Tuple:
        ...
        return (
            sample, 
            {
                'total_sequences': total_sequences, 'total_bases': total_bases, 
                'gc_count': gc_count, 'min_length': min_length, 'max_length': max_length
            }
        )    
...
class FastQC:
    ...
    Attributes:
        fastq_paths:         Dictionary mapping sample names to FASTQ file paths.
        output_dir:          Directory to save results.
        fastqc_path:         Path to FastQC executable.
        max_workers:         Maximum number of parallel workers.
        clean_intermediates: Whether to remove intermediate files.
        encoding:            Text encoding for subprocess outputs.
    """
    def __init__(
        self, fastq_paths: Dict[str, List[Path]], output_dir: Union[str, Path],
        fastqc_path: Union[str, Path] = DEFAULT_FASTQC_PATH,
        max_workers: int = DEFAULT_MAX_WORKERS,
        clean_intermediates: bool = True, encoding: str = 'utf-8'
        ):
    ...
    def _validate_environment(self):
        ...
        try:
            subprocess.run(
                [str(fastqc_bin), '-v'], check=True, capture_output=True, text=True
            )
        except subprocess.CalledProcessError as e:
            raise RuntimeError(f"FastQC validation failed: {e.stderr.decode().strip()}")

    def run_pipeline(self) -> "FastQC":
        """Execute complete analysis pipeline."""
        self._run_fastqc_analyses()
        self._parse_results()
        if self.clean_intermediates: self._cleanup_intermediates()
    ...
    def _run_fastqc_analyses(self):
        ...
        # Get Conda environment's bin path
        conda_prefix = os.environ.get("CONDA_PREFIX")
        if not conda_prefix: raise RuntimeError("Conda environment not activated!")
        ...
        def process_file(sample: str, fastq_path: Path):
            cmd = [
                str(fastqc_bin), '-o', str(self.results_dir), '-f', 'fastq',
                '-q', str(fastq_path)
            ]
            try:
                subprocess.run(
                    cmd, check=True, capture_output=True,
                    text=True, encoding=self.encoding
                )
                return True
            except subprocess.CalledProcessError as e:
                logger.error(f"Failed processing {sample}: {fastq_path.name}\nError: {e.stderr.strip() or e.stdout.strip()}")
                return False

        tasks = [
            (sample, path) for sample, paths in self.fastq_paths.items()
            for path in paths
        ]
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            with get_progress_bar() as prog:
                task = prog.add_task("Running FastQC...", total=len(futures))
                ...
                for future in as_completed(futures):
                    ...
                    finally: prog.update(task, advance=1)
    ...
    def _parse_sample_name(self, zip_path: Path) -> Tuple[str, str]:
        ...
        if match: return match.groups()  # type: ignore
        return (zip_path.stem, 'unknown')

    def _parse_fastqc_data(
        self, fastqc_data_path: Union[str, Path, zipfile.ZipExtFile, io.IOBase, io.BufferedIOBase]
    ) -> Dict:
        ...
        if isinstance(fastqc_data_path, (str, Path)):
            ...
            with TextIOWrapper(io.BufferedReader(fastqc_data_path), encoding=self.encoding) as text_file: # type: ignore
                lines = text_file.readlines()

        for line in lines:
            line = line.strip()
            if line.startswith(">>"):
                if line.startswith(">>END_MODULE"): current_section = None
                else:
                    current_section = next((k for k, v in sections.items() 
                                            if line.startswith(v["start"])), None)
                continue
    ...
    def _parse_results(self):
            ...
            try:
                ...
                with zipfile.ZipFile(zip_path) as zf:
                    ...
                    with zf.open(data_file) as binary_file:
                        buffered_reader = io.BufferedReader(binary_file) # type: ignore
                        result = self._parse_fastqc_data(buffered_reader)
        ...
        self.parsed_data = {section: pd.concat(dfs).reset_index(drop=True)
    ...
    def _cleanup_intermediates(self):
        ...
        try: self.results_dir.rmdir()
        except OSError: logger.warning(f"Directory not empty: {self.results_dir}")
    ...
    def _generate_plots(self):
        ...
        try:
            ...
            plots.export_figures(export_dir=str(plot_dir))
            self.figs.update(plots.figs) # type: ignore
        except ImportError:
    ...
    def get_figures(self) -> Dict[str, plt.Figure]: # type: ignore
