...
from workflow_16s.upstream.metadata.utils import create_anndata_from_qiime_artifacts, validate_anndata_file
...
def format_bytes(size_in_bytes):
    """
    Mock function. Replace with your import:
    from workflow_16s.utils.formatting import format_bytes
    """
...
def process_qiime_to_anndata(qiime_base_dir, metadata_base_dir, output_dir):
    """
    ...
    for feature_table_biom_path in qiime_path.glob("**/table/feature-table.biom"):
        ...
        try:
            ...
            logger.warning(f"Could not determine relative path for {analysis_root}. Skipping.")
            continue
            
        logger.info(f"Found potential analysis set: {relative_path}")

        # 1. --- Find other required artifact paths ---
        ...
        # 2. --- Find metadata path ---
        metadata_file_path = metadata_path / relative_path / "sample-metadata.tsv"

        # 3. --- Validate all paths ---
        if not rep_seqs_fasta_path.exists():
            logger.warning(f"  - Missing rep-seqs: {rep_seqs_fasta_path}. Skipping.")
            continue
        
        if not taxonomy_paths:
            logger.warning(f"  - Missing taxonomy: No '*/taxonomy/taxonomy.tsv' found. Skipping.")
            continue
            
        if len(taxonomy_paths) > 1:
            logger.warning(f"  - Found multiple taxonomy files. Using first one: {taxonomy_paths[0]}")
        taxonomy_tsv_path = taxonomy_paths[0] # Use the first one found
            
        if not metadata_file_path.exists():
            logger.warning(f"  - Missing metadata: {metadata_file_path}. Skipping.")
            continue
            
        logger.info(f"  ✅ Found all required files for: {relative_path}")

        # 4. --- Construct output path and check if it exists ---
        subset_id = str(relative_path).replace(os.sep, '.').upper()
        anndata_path = output_path / f"{subset_id}.h5ad"
        
        if anndata_path.exists():
            logger.info(f"  - Output file {anndata_path} already exists. Skipping.")
            continue
            
        # 5. --- Create and write AnnData object ---
        try:
            ...
            if anndata_path.exists():
                ...
                # This error is from your snippet
            ...
            if anndata_path.exists():
                anndata_path.unlink()
                logger.info(f"Removed partial file: {anndata_path}")
