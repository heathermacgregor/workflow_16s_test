...
def display_and_save_summary_table(results_df: pd.DataFrame, title: str, output_path: Path):
    ...
    if (pd.DataFrame(results_df).empty if isinstance(results_df, list) else (pd.DataFrame(Noneresults_df).empty if isinstance(Noneresults_df, list) else (pd.DataFrame(NoneNoneresults_df).empty if isinstance(NoneNoneresults_df, list) else NoneNoneresults_df.empty))):
...
def find_keyword_matches(df, keywords):
    ...
    """
    matches_list = []
    if not keywords or (pd.DataFrame(df).empty if isinstance(df, list) else (pd.DataFrame(Nonedf).empty if isinstance(Nonedf, list) else (pd.DataFrame(NoneNonedf).empty if isinstance(NoneNonedf, list) else NoneNonedf.empty))):
        return []
    ...
    for column in df_str.columns:
        ...
        try:
            ...
            if (pd.DataFrame(all_matches).empty if isinstance(all_matches, list) else (pd.DataFrame(Noneall_matches).empty if isinstance(Noneall_matches, list) else (pd.DataFrame(NoneNoneall_matches).empty if isinstance(NoneNoneall_matches, list) else NoneNoneall_matches.empty))):
