# workflow_16s/upstream/metadata/update_h5ad_files.py
import logging
import re
import pandas as pd
...
# --- Constants ---
# TODO: Email should be read from config
...
def report_metadata_stats(df: pd.DataFrame, context: str):
    ...
    if (pd.DataFrame(df).empty if isinstance(df, list) else (pd.DataFrame(Nonedf).empty if isinstance(Nonedf, list) else (pd.DataFrame(NoneNonedf).empty if isinstance(NoneNonedf, list) else NoneNonedf.empty))):
...
def clean_lat_lon(df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
    ...
    for col in combo_cols:
        ...
        if not (pd.DataFrame(extracted).empty if isinstance(extracted, list) else (pd.DataFrame(Noneextracted).empty if isinstance(Noneextracted, list) else (pd.DataFrame(NoneNoneextracted).empty if isinstance(NoneNoneextracted, list) else NoneNoneextracted.empty))):
...
def enrich_env_data(df: pd.DataFrame, config: AppConfig) -> pd.DataFrame:
    ...
    try:
        ...
        if not (pd.DataFrame(env_results_df).empty if isinstance(env_results_df, list) else (pd.DataFrame(Noneenv_results_df).empty if isinstance(Noneenv_results_df, list) else (pd.DataFrame(NoneNoneenv_results_df).empty if isinstance(NoneNoneenv_results_df, list) else NoneNoneenv_results_df.empty))):
...
def enrich_arkin_agents(df: pd.DataFrame, config: AppConfig) -> pd.DataFrame:
    ...
    try:
        ...
        if not (pd.DataFrame(arkin_results_df).empty if isinstance(arkin_results_df, list) else (pd.DataFrame(Nonearkin_results_df).empty if isinstance(Nonearkin_results_df, list) else (pd.DataFrame(NoneNonearkin_results_df).empty if isinstance(NoneNonearkin_results_df, list) else NoneNonearkin_results_df.empty))):
...
def enrich_nfc(df: pd.DataFrame, nfc_handler: NFCFacilitiesHandler) -> pd.DataFrame:
    ...
    try:
        ...
        if not (pd.DataFrame(nfc_results_df).empty if isinstance(nfc_results_df, list) else (pd.DataFrame(Nonenfc_results_df).empty if isinstance(Nonenfc_results_df, list) else (pd.DataFrame(NoneNonenfc_results_df).empty if isinstance(NoneNonenfc_results_df, list) else NoneNonenfc_results_df.empty))):
...
def process_file(
    ...
    # 5. Merge Metadata
    if not (pd.DataFrame(ena_df).empty if isinstance(ena_df, list) else (pd.DataFrame(Noneena_df).empty if isinstance(Noneena_df, list) else (pd.DataFrame(NoneNoneena_df).empty if isinstance(NoneNoneena_df, list) else NoneNoneena_df.empty))):
