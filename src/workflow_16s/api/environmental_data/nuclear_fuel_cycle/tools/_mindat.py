# workflow_16s/api/environmental_data/nuclear_fuel_cycle/tools/_mindat.py
...
class MinDatAPI:
    ...
    def __init__(self, api_key: str, user_agent: str, output_dir: Optional[Union[str, Path]] = None,
        ...
        # STANDARDIZED: Store API key as instance variable instead of writing to os.environ
...
def world_uranium_mines(config: AppConfig):
    ...
    if project_dir:
        ...
        if use_local and output_path.exists():
            get_logger("workflow_16s").info(f"Using local MinDat data from {output_path}")
