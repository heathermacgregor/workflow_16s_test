# workflow_16s/api/environmental_data/nuclear_fuel_cycle/tools/_iaea.py
...
class NFCFDB:
    ...
    def load(self) -> pd.DataFrame:
        ...
        if not self.file_path.exists():
            self.logger.info("NFCIS file not found. Attempting download...")
            try:
                self._download()
            except Exception as e:
                self.logger.error(f"Failed to download NFCIS data: {e}")
                return pd.DataFrame()

        # 2. Check again after download attempt
        if not self.file_path.exists():
            self.logger.warning(f"NFCIS file missing at {self.file_path}. Skipping NFCIS data.")
            return pd.DataFrame()
        ...
        try:
            ...
            self.df = df
            self.logger.info(f"Successfully loaded NFCIS data with {self.df.shape[0]} facilities.")
            return self.df

        except Exception as e:
            self.logger.error(f"Error parsing NFCIS Excel data: {e}")
            return pd.DataFrame()
    ...
    def _download(self, headless: bool = True, timeout: int = 90):
        ...
        driver = None
        try:
            self.logger.info("Initializing Headless Chrome for NFCIS download...")
            ...
            download_btn.click()
            self.logger.info("Clicked download button. Waiting for file...")
                ...
                if files:
                    ...
                    if not latest_file.endswith(".crdownload"):
                        ...
                        if os.path.getctime(latest_file) > (time.time() - timeout):
                            ...
                            self.logger.info(f"Download complete: {self.file_path}")
                            return
                time.sleep(2)
            ...
            self.logger.warning(f"NFCIS Scraper skipped: {e}")
            raise e
