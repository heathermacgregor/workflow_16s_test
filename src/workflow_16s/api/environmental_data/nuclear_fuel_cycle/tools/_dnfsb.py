# workflow_16s/api/environmental_data/nuclear_fuel_cycle/tools/_dnfsb.py
...
class DNFSBFacilityDB:
    ...
    def load(self) -> pd.DataFrame:
        """Alias for run() to maintain compatibility with the orchestrator."""
        return self.run()
    ...
    def run(self) -> pd.DataFrame:
        ...
        try:
            ...
            self.logger.error(f"DNFSB Scraper failed: {e}")
            return pd.DataFrame()

        if not facilities:
            self.logger.warning("No DNFSB facilities found. Returning empty dataset.")
            return pd.DataFrame()

        self.logger.info(f"Found {len(facilities)} facilities. Fetching coordinates...")
        ...
        self.logger.info(f"Loaded {self.result.shape[0]} DNFSB facilities")
        if self.output_path:
    ...
    def _fetch_facility_list(self) -> List[dict]:
        ...
        try:
            ...
            self.logger.warning(f"Could not access DNFSB site list ({self.SITE_LIST_URL}): {e}")
            return []
        ...
        if table:
            ...
            self.logger.warning("Could not find facility table on DNFSB page.")
    ...
    def _fetch_facility_coordinates(self, facility_url: Optional[str]) -> tuple:
        ...
        try:
            ...
            # self.logger.debug(f"Failed to fetch coordinates for {facility_url}: {e}")
            return None, None
