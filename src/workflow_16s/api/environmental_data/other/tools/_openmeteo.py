...
_air_quality_old_date_logged = False
_pollen_old_date_logged = False


def _is_valid_date(date_str: Optional[str]) -> bool:
    """Check if a date string is valid (not nan, None, empty, or malformed).

    Args:
        date_str: Date string to validate (should be YYYY-MM-DD format)

    Returns:
        True if date is valid and usable, False otherwise
    """
    ...
    if isinstance(date_str, str):
        ...
        if date_lower == "nan" or date_lower == "nat":
            return False
...
class EnvironmentalHealthAPI(BaseEnvironmentalAPI):
    ...
    def get_data( # type: ignore
        ...
        """
        global _air_quality_old_date_logged, _pollen_old_date_logged
        
        combined_forecast = {}

        # Early validation: Skip if fetch_date is invalid (nan, None, empty, malformed)
        if fetch_date is not None and not _is_valid_date(fetch_date):
            return {}
        ...
        if fetch_date:
            ...
            try:
                ...
                if fetch_datetime < AIR_QUALITY_START_DATE:
                    # Log only once per batch to avoid repetition
                    if not _air_quality_old_date_logged:
                        logger.debug(
                            f"Air quality data not available for dates before {AIR_QUALITY_START_DATE.strftime('%Y-%m-%d')} "
                            f"(batch contains {fetch_date}). Skipping air quality requests for older samples."
                        )
                        _air_quality_old_date_logged = True
