...
# Track JSON parsing failures to report in batch rather than per-location
_json_parse_failures = {}  # key: error message, value: count
...
class USGSGeologicUnitsAPI(BaseEnvironmentalAPI):
    ...
    def _fetch_data(self, lat: float, lon: float) -> Dict[str, Any]:
        ...
        """
        global _json_parse_failures, _empty_response_count
        ...
        try:
            ...
            if response.status_code != 200:
                logger.debug(f"USGS Geology API returned status {response.status_code} for ({lat}, {lon})")
                return {
                    'unit_found': False,
                    'unit_name': None,
                    'unit_age': None,
            ...
            # Empty response is expected for non-US locations; don't log as warning
            if not response.text or len(response.text.strip()) == 0:
                _empty_response_count += 1
                if self.verbose and _empty_response_count == 1:
                    logger.debug(f"USGS Geology API: Empty responses expected for non-US locations")
                return {
                    'unit_found': False,
                    'unit_name': None,
                    'unit_age': None,
            ...
            # Check for HTML error pages (common when API returns error/redirect)
            response_text_lower = response.text.lower()
            if '<html' in response_text_lower or '<body' in response_text_lower:
                logger.debug(f"USGS Geology API returned HTML (likely error page) for ({lat}, {lon})")
                return {
                    'unit_found': False,
                    'unit_name': None,
                    'unit_age': None,
            ...
            try:
                ...
                # Track JSON parsing failures to report in batch
                error_msg = str(je)[:50]  # First 50 chars of error
                _json_parse_failures[error_msg] = _json_parse_failures.get(error_msg, 0) + 1
                
                # Only log first occurrence of each unique error
                if _json_parse_failures[error_msg] == 1:
                    logger.debug(
                        f"USGS Geology API returned invalid JSON: {error_msg}"
                        f" (additional similar errors will be suppressed)"
                    )
                return {
                    'unit_found': False,
                    'unit_name': None,
