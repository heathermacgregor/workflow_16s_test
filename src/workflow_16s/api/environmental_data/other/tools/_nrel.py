# workflow_16s/api/environmental_data/other/tools/_nrel.py
...
from .constants import is_us_location
from workflow_16s.utils.logger import get_logger, with_logger
from workflow_16s.utils.credential_utils import get_credential, get_email
...
class NREL_Solar_API(BaseEnvironmentalAPI):
    ...
    Attributes:
        ...
        api_key (str): NREL API key from config or NREL_API_KEY environment variable.
        email (str): Contact email from config or EMAIL environment variable.
    """
    ...
    def __init__(self, config: Optional[Any] = None, verbose: bool = False):
        ...
        # STANDARDIZED: Get credentials from config first, then environment variables
        if config is not None:
            self.api_key = get_credential(config, 'nrel_api_key', 'NREL_API_KEY', required=False)
            try:
                self.email = get_email(config, 'email')
            except ValueError:
                self.email = os.getenv("EMAIL")
        else:
            # Fallback for backward compatibility if no config provided
    ...
    def get_data( # type: ignore
        ...
        """
        # US-only dataset: validate location is within US service area
        if not is_us_location(lat, lon):
            logger.debug(f"NREL Solar: Location ({lat:.4f}, {lon:.4f}) outside US service area")
            return None
...
if __name__ == "__main__":
    ...
    # NOTE: Pass config=None to use environment variables
    api_client = NREL_Solar_API(config=None, verbose=True)
