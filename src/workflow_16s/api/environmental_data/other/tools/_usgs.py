# workflow_16s/api/environmental_data/other/tools/_usgs.py
...
REQUEST_TIMEOUT = 30 # Increased timeout for potentially large requests
...
class USGS_Water_Services_API(BaseEnvironmentalAPI):
    ...
    def get_data( # type: ignore
        ...
        if not is_us_location(lat, lon):
            logger.debug(f"USGS Water: Location ({lat:.4f}, {lon:.4f}) outside US service area")
            return None
        ...
        try:
            ...
            if fetch_date:
                # Early check for invalid/missing dates (silent, no logging)
                if fetch_date is None or pd.isna(fetch_date) or str(fetch_date).lower() in ['nan', 'nat', '']:
                    pass  # Don't add date params, use latest data
                else:
                    try:
                        date_obj = datetime.strptime(fetch_date, '%Y-%m-%d')
