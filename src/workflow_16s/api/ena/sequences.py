...
class MetadataFetcher:
    ...
    def __init__(
        self, base_url: str = "https://www.ebi.ac.uk/ena/portal/api",
        retries: int = 5, backoff_factor: int = 1,
    ...
    def get_sample_metadata(self, ena_sample_accession: str) -> pd.DataFrame:
        ...
        params = {"result": "sample", "query": f'accession="{ena_sample_accession}"'}
        return self._get_data("search", params)

    def get_sample_metadata_concurrent(
        self, sample_task: TaskID, ena_sample_accessions: List[str],
        max_workers: int = 5
    ) -> pd.DataFrame:
        """
    ...
    def get_study_and_sample_metadata(
        ...
        with self.track():
            parent_task = self.progress.add_task(
                f"Processing {ena_study_accession}", total=3
            )

            try:
                # Get study metadata
                study_task = self.progress.add_task(
                    "Fetching study metadata", parent=parent_task, total=1
                )
                ...
                sample_task = self.progress.add_task(
                    "Fetching sample metadata", parent=parent_task,
                    total=len(samples)
                )
                sample_df = self.get_sample_metadata_concurrent(sample_task, samples, 
                                                                max_workers)
                ...
                merged_df = study_df.merge(
                    sample_df, on="sample_accession",
                    how="left", suffixes=("_study", "")
                )
...
class SequenceFetcher:
    ...
    def __init__(
        self, fastq_dir: Union[str, Path], retries: int = 10,
        initial_delay: int = 5, max_workers: int = 8
    ):
    ...
    def get_run_fastq(
        ...
        Args:
            ...
            urls:          List of FTP URLs for the FASTQ files.
        
        Returns:
            Mapping of run accession to list of downloaded file paths.
        """
        file_paths = []
        for url_idx, url in enumerate(urls, 1):
            if not url or str(url).lower() == "nan": continue
    ...
    def download_run_fastq_concurrent(
        ...
        with self.progress:
            main_task = self.progress.add_task(
                "Downloading sequencing data", total=len(metadata)
            )

            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                futures = {executor.submit(
                    self.process_run, str(row.run_accession),
    ...
    def process_run(self, run_accession: str, 
...
class PooledSamplesProcessor:
    ...
    def _create_lookup_dict(self):
        ...
        self.lookup_dict = {
            ...
            ): row["#SampleID"] for _, row in self.metadata.iterrows()
        }
    ...
    def process_all(self, raw_data_dir: Union[str, Path]):
        ...
        with self.progress:
            main_task = self.progress.add_task(
                "Processing pooled files", total=len(fastq_files)
            )
