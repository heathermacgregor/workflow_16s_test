# workflow_16s/visualization/machine_learning/shap.py
...
def shap_summary_bar(
    ...
    top_indices = np.argsort(mean_abs_shap)[-max_display:][::-1].tolist()
    
    # Handle multi-class nested lists gracefully
    if isinstance(top_indices[0], (list, np.ndarray)):
        top_indices = [int(i) for i in top_indices[0]]
    else:
        top_indices = [int(i) for i in top_indices]
 # Ensure python ints (now safe after tolist())
    ...
    fig.update_layout(
        autosize=True, showlegend=False, height=height+100, width=width, 
        yaxis=dict(
            title=dict(standoff=100), automargin=True, showticklabels=True
        )
    )
...
def shap_beeswarm(
    ...
    for idx, feature_idx in enumerate(top_indices):
        ...
        try:
            ...
            hover_text = [f"<b>Feature</b>: {feature_names[feature_idx]}<br><b>SHAP</b>: {str(shap_val)}<br><b>Value</b>: {str(fv)}" for shap_val, fv in zip(shap_vals, feat_vals_numeric)]

        except (ValueError, TypeError):
            colors = '#999999' # Neutral grey for strings
            hover_text = [f"<b>Feature</b>: {feature_names[feature_idx]}<br><b>SHAP</b>: {str(shap_val)}<br><b>Value</b>: {fv}" for shap_val, fv in zip(shap_vals, feat_vals)]
...
def shap_dependency_plot(
    ...
    fig = update_font_sizes(fig)
    title = f'SHAP Dependency Plot: {feature_display}{title_suffix}'
    title = plot_utils.add_line_breaks(title, max_length=40)
...
def shap_heatmap(
    ...
    fig = go.Figure(
        go.Heatmap(
            ...
            hoverinfo="text", 
            text=hover_text, 
            colorbar=dict(title=dict(text='SHAP Value', font=dict(size=14)),)
        )
    )
    fig = update_font_sizes(fig)
    ...
    for i in range(len(top_features)):
        ...
        fig.add_trace(
            go.Bar(
                ...
                hoverinfo='text', 
                text=(f"<b>Feature</b>: {top_features[i]}<br><b>Value</b>: {val_str}<br><b>SHAP</b>: {contribution:.4f}<br><b>Cumulative</b>: {end_val:.4f}")
            )
        )
...
def plot_shap(
    ...
    logger = get_logger("workflow_16s")
    plot_utils = PlottingUtils(logger)
    figures_saved = []  # Track what was saved for summary logging
    ...
    if can_save and output_path_base:
        save_fig(
            ...
            verbose=False  # Don't log individual saves
        )
        figures_saved.append('summary.bar')
    ...
    if is_multiclass_avg:
        ...
        if can_save and output_path_base:
            ...
            figures_saved.append('summary.beeswarm')
        ...
        if can_save and output_path_base:
            ...
            figures_saved.append('summary.heatmap')
        ...
        if can_save and output_path_base:
            ...
            figures_saved.append('summary.force')
        ...
        if can_save and output_path_base:
            ...
            figures_saved.append('summary.waterfall')
        ...
        if shap_interaction_values is not None:
            try:
                ...
                if can_save and output_path_base:
                    ...
                    figures_saved.append('interactions.heatmap')
        ...
        if n_features > 0:
            for feature in top_full_features[:n_features]:
                try:
                    ...
                    if can_save and output_path_base:
                        save_fig(fig=dep_fig, output_path=output_path_base / f"shap.dependency.{feature}", formats=['png', 'html', 'json'], verbose=False)
        ...
        if saved_features:
            figures_saved.append(f"dependency ({len(saved_features)} features)")
    
    # Log summary at end
    if can_save and output_path_base and figures_saved:
        logger.info(f" 💾 Saved SHAP visualizations to {output_path_base}/: {', '.join(figures_saved)} ({len(figures_saved)} figure types, 3 formats each)")
