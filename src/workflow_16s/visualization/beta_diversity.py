...
from workflow_16s.visualization import PlotlyScatterPlot
...
def beta_diversity_plot(
    ...
    ordination_type: str,
    proportion_explained: Optional[np.ndarray] = None,
    ...
    # Create plot
    obj = PlotlyScatterPlot(components, metadata, color_col, symbol_col)
    obj.create_fig(x_col, y_col, x_title, y_title, hover_data=['#sampleid', color_col, symbol_col])
    if output_path: 
