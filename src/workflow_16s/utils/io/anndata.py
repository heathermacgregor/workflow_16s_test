...
def create_anndata_from_qiime_artifacts(
    ...
    try:
        ...
        # Create lightweight DataFrames for obs and var to hold the names
        obs_df = pd.DataFrame(index=biom_table.ids(axis='sample'))
        var_df = pd.DataFrame(index=biom_table.ids(axis='observation'))

        # Initialize AnnData without the problematic keyword arguments
        adata = ad.AnnData(
            X=sparse_matrix,
            obs=obs_df,
            var=var_df
