...
class MetadataManager:
        ...
        with get_progress_bar() as progress:
            ...
            for path in metadata_paths:
                try:
                    ...
                    get_logger("workflow_16s").error(f"Failed to load metadata file {path}: {e!r}")
    ...
    def _clean_sample_ids(self) -> None:
        if self.sample_id_column not in self.df.columns:
            alternatives = ['#sampleid', 'sample_id', 'sample id', 'sample name', 'run_accession']
            found_col = next((alt for alt in alternatives if alt in self.df.columns), None)
            if found_col:
    ...
    def _apply_custom_filters(self) -> None:
        if 'empo_3' in self.df.columns:
            ...
            if rows_removed > 0:
                ...
                self.report['custom_filters_applied'] = {'column': 'empo_3', 'rows_removed': rows_removed}
    ...
    def _parse_column_unit(self, col_name: str) -> Tuple[Optional[str], Optional[str]]:
        for unit, pattern in self.DEFAULT_UNIT_PATTERNS.items():
            ...
            if match: return col_name[:match.start()], unit
        return None, None
    ...
    def _standardize_units(self) -> None:
        ...
        for col in self.df.columns:
            ...
            if base_name_raw and unit:
                measurement_key = next((key for key in self.DEFAULT_MEASUREMENT_STANDARDS if key in base_name_raw), base_name_raw)
    ...
    def _define_ontology_maps(self) -> None:
        self.ONTOLOGY_MAP = {
            'empo_1': {
                'Host-associated': ['host', 'symbiont', 'microbiome', 'human', 'animal'],
                'Free-living': ['free living', 'environmental', 'soil', 'water', 'sediment', 'air']
            },
            'empo_2': {
    ...
    def _process_geolocation(self) -> None:
        ...
        lat_sources = [c for c in self.DEFAULT_COORDINATE_SOURCES['lat'] if c in self.df.columns]
        lon_sources = [c for c in self.DEFAULT_COORDINATE_SOURCES['lon'] if c in self.df.columns]
        
        if lat_sources:
            for col in lat_sources: lat.fillna(pd.to_numeric(self.df[col], errors='coerce'), inplace=True)
        if lon_sources:
            for col in lon_sources: lon.fillna(pd.to_numeric(self.df[col], errors='coerce'), inplace=True)
        ...
        self.logger.info(f"Geolocation: {initial_count} initial -> {len(self.df)} valid. ({dropped_count} dropped).")
    ...
    def _process_ontology(self) -> None:
        ...
        for term_category, term_map in self.ONTOLOGY_MAP.items():
            if term_category not in self.df.columns or self.df[term_category].isnull().all():
                self.df[term_category] = search_text_series.apply(self._infer_ontology_term, term_map=term_map)
                self.logger.debug(f"Inferring ontology for '{term_category}'.")
    ...
    async def _enrich_location_from_coords(self, session: aiohttp.ClientSession) -> None:
        ...
        self.logger.info(f"Found {len(rows_to_check)} rows to enrich with geocoding...")
        semaphore = asyncio.Semaphore(1) 
        
        geolocator = Nominatim(user_agent="metadata_analysis_script_v3", adapter_factory=session.get)
    ...
    async def _fetch_single_location(self, geolocator, semaphore, index, lat, lon) -> Tuple[int, Optional[str]]:
        try:
            ...
            self.logger.warning(f"Geocoding service error for index {index}: {e}")
        return index, None
    ...
    async def _convert_envo_codes(self, session: aiohttp.ClientSession) -> None:
        ...
        all_unique_values = pd.concat([self.df[c].dropna() for c in existing_cols]).astype(str).unique()
        ...
        self.logger.info(f"Found {len(all_codes_to_lookup)} unique ENVO codes to look up.")
        
        tasks = [self._fetch_envo_label(session, code) for code in all_codes_to_lookup]
        results = await asyncio.gather(*tasks)
        
        code_to_label_map = {code: label for code, label in results if label}

        for col in existing_cols:
            self.df[col] = self.df[col].astype(str).replace(code_to_label_map, regex=True)
    ...
    async def _fetch_envo_label(self, session, code) -> Tuple[str, Optional[str]]:
        ...
        try:
            ...
            self.logger.warning(f"OLS API request failed for {code}: {e}")
        return code, None

    async def _find_publications(self, session: aiohttp.ClientSession, api_key: Optional[str] = None) -> None:
        ...
        priority = ['project', 'study', 'biosample', 'sra', 'ena', 'sample', 'experiment', 'run']
        sorted_acc_cols = sorted(all_acc_cols, key=lambda c: next((i for i, p in enumerate(priority) if p in c.lower()), len(priority)))
        ...
        self.logger.info(f"Found {len(unique_accessions)} unique accessions to check for publications.")
        
        tasks = [self._fetch_single_doi(session, acc, api_key) for acc in unique_accessions]
        results = await asyncio.gather(*tasks)
    ...
    async def _fetch_single_doi(self, session, accession, api_key) -> Tuple[str, Optional[str]]:
                    ...
                    try:
                        async with session.get(url, timeout=60) as resp:
                            if resp.status == 429:
                                ...
                                self.logger.warning(f"Rate limited for {accession}. Retrying in {retry_after} seconds... (Attempt {attempt + 1}/{max_retries})")
                        ...
                        self.logger.warning(f"NCBI request for {accession} failed: {e}", exc_info=True)
                        await asyncio.sleep(1 * (attempt + 1))
                
            self.logger.error(f"All {max_retries} retries failed for NCBI request: {accession}")
            return None
        ...
        if pmids:
            ...
            if summary_root is not None and (doi_elem := summary_root.find(".//Item[@Name='DOI']")) is not None and doi_elem.text:
                self.logger.info(f"SUCCESS: Found DOI {doi_elem.text} for {accession}")
        ...
        with get_progress_bar() as progress:
            ...
            for col in categorical_cols:
                try:
                    ...
                    value_counts = self.df[col].astype(str).str.lower().str.strip().dropna().value_counts()
                    unique_values = value_counts.index.tolist()
                    
                    groups, processed_values = [], set()
                    for val in unique_values:
                        if val in processed_values: continue
                        current_group = {o for o in unique_values if fuzz.ratio(val, o) >= similarity_threshold}
                    ...
                    for group in groups:
                        if len(group) > 1:
                            canonical_val = max(group, key=lambda v: value_counts.get(v, 0))
                            for val in group:
