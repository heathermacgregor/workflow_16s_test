...
from scipy.sparse import issparse  # <-- ADD THIS IMPORT
...
class PlottingUtils:
    ...
    def save_plotly_fig(self, fig: go.Figure, filepath: Path, batch: bool = False, scale: int = None, save_png: bool = False, **kwargs):
        ...
        Args:
            ...
            filepath: Destination base path (WITHOUT extension). e.g., 'output/my_plot' will create 'my_plot.html', 'my_plot.png', and 'my_plot.json'.
            batch: If True, queues the plot for batch saving instead of immediate save.
            scale: Scale factor for PNG export (default: 2 for ~200 DPI, faster rendering).
            save_png: If True, saves PNG files. If False, skips PNG generation to avoid Kaleido timeouts (default: False).
            **kwargs: Extra args (like 'width', 'height') passed to fig.write_image.
        """
