# workflow_16s/src/workflow_16s/downstream/statistics/batch_effects.py
"""
...
def detect_batch_effects(
    ...
    try:
        ...
        dm = beta_diversity(
            distance_metric,
            adata.X.toarray() if hasattr(adata.X, 'toarray') else adata.X, # type: ignore
            ids=adata.obs_names
        )
    ...
    try:
        X = adata.X.toarray() if hasattr(adata.X, 'toarray') else adata.X # type: ignore
    ...
    try:
        X = adata.X.toarray() if hasattr(adata.X, 'toarray') else adata.X # type: ignore
        pca = PCA(n_components=min(10, X.shape[1])) # type: ignore
