...
def _check_shap_installed():
    ...
    try: import shap  
    except ImportError: raise ImportError("SHAP library is not installed. Please install with: pip install shap")
    

def _validate_inputs(X_train, y_train, X_test, y_test):
    """Validate input alignment and data integrity."""
    if X_train.shape[0] != y_train.shape[0] or X_test.shape[0] != y_test.shape[0]:
        raise ValueError(f"X_train [{X_train.shape[0]}], y_train [{y_train.shape[0]}], X_test [{X_test.shape[0]}], and y_test [{y_test.shape[0]}] must have the same number of samples")
...
def filter_data(X: pd.DataFrame, y: pd.Series, metadata: pd.DataFrame,
                group_col: str = DEFAULT_GROUP_COLUMN,
                test_size: float = DEFAULT_TEST_SIZE,
                random_state: int = DEFAULT_RANDOM_STATE) -> Tuple[pd.DataFrame, pd.DataFrame, 
                                                                   pd.Series, pd.Series]:
...
def rfe_feature_selection(X_train: pd.DataFrame, y_train: pd.Series, X_test: pd.DataFrame,
                          y_test: pd.Series, num_features: int, step_size: int, threads: int,
                          random_state: int, iterations: int = DEFAULT_ITERATIONS_RFE,
                          learning_rate: float = DEFAULT_LEARNING_RATE_RFE,
                          depth: int = DEFAULT_DEPTH_RFE, verbose: int = 1,
                          catboost_params: Optional[Dict] = None) -> Tuple[pd.DataFrame, 
                                                                           pd.DataFrame, 
                                                                           List[str]]:
    """Perform Recursive Feature Elimination (RFE) using CatBoostClassifier."""
    # Input validation
    _validate_inputs(X_train, y_train, X_test, y_test)
    if num_features <= 0 or step_size <= 0: raise ValueError("num_features and step_size must be positive integers.")
    if num_features > X_train.shape[1]: raise ValueError("num_features cannot be greater than the total number of features.")
    if threads <= 0: raise ValueError("threads must be a positive integer.")
    if iterations <= 0 or depth <= 0: raise ValueError("iterations and depth must be positive integers.")
    if learning_rate <= 0: raise ValueError("learning_rate must be a positive float.")
    
    # Initialize CatBoostClassifier
    catboost_params = catboost_params or {}
    rfe_model = CatBoostClassifier(iterations=iterations, 
                                   learning_rate=learning_rate,
                                   depth=depth, thread_count=threads,
                                   random_state=random_state,
                                   verbose=verbose > 1,
                                   **catboost_params)
    
    # Perform RFE
    rfe = RFE(estimator=rfe_model, n_features_to_select=num_features,
              step=step_size, verbose=verbose)
    ...
    X_train_selected = pd.DataFrame(rfe.transform(X_train), 
                                    ...
                                    index=X_train.index)
    X_test_selected = pd.DataFrame(rfe.transform(X_test),
                                   columns=X_train.columns[rfe.support_],
                                   index=X_test.index)
    
    # Get selected features
    selected_features = X_train.columns[rfe.support_].tolist()
    if verbose: logger.debug(f"Selected {len(selected_features)} features using RFE")
    
    return X_train_selected, X_test_selected, selected_features


def select_k_best_feature_selection(X_train: pd.DataFrame, y_train: pd.Series,
                                    X_test: pd.DataFrame, y_test: pd.Series,
                                    num_features: int, score_func: Callable = f_classif,
                                    verbose: int = 1) -> Tuple[pd.DataFrame, pd.DataFrame, 
                                                               List[str]]:
    """Perform feature selection using SelectKBest."""
    # Input validation
    _validate_inputs(X_train, y_train, X_test, y_test)
    if num_features <= 0: raise ValueError("num_features must be a positive integer.")
    if num_features > X_train.shape[1]: raise ValueError("num_features cannot be greater than the total number of features.")
    ...
    # Transform datasets
    X_train_selected = pd.DataFrame(skb.transform(X_train),
                                    columns=X_train.columns[skb.get_support()],
                                    index=X_train.index)
    X_test_selected = pd.DataFrame(skb.transform(X_test),
                                   columns=X_train.columns[skb.get_support()],
                                   index=X_test.index)
    
    # Get selected features
    selected_features = X_train.columns[skb.get_support()].tolist()
    if verbose: logger.debug(f"Selected {num_features} features using {score_func.__name__}")
    
    return X_train_selected, X_test_selected, selected_features


def chi_squared_feature_selection(X_train: pd.DataFrame, y_train: pd.Series,
                                  X_test: pd.DataFrame, y_test: pd.Series,
                                  num_features: int,
                                  verbose: int = 1) -> Tuple[pd.DataFrame, 
                                                             pd.DataFrame, 
                                                             List[str]]:
    ...
    if num_features <= 0: raise ValueError("num_features must be a positive integer.")
    if num_features > X_train.shape[1]: raise ValueError("num_features cannot be greater than the total number of features.")
    if (X_train < 0).any().any() or (X_test < 0).any().any(): raise ValueError("Chi-Squared test requires non-negative feature values.")
    ...
    # Transform datasets
    X_train_selected = pd.DataFrame(chi2_selector.transform(X_train),
                                    columns=X_train.columns[chi2_selector.get_support()],
                                    index=X_train.index)
    X_test_selected = pd.DataFrame(chi2_selector.transform(X_test),
                                   columns=X_train.columns[chi2_selector.get_support()],
                                   index=X_test.index)
    
    # Get selected features
    selected_features = X_train.columns[chi2_selector.get_support()].tolist()
    if verbose: logger.debug(f"Selected {num_features} features using Chi-Squared Test")
    
    return X_train_selected, X_test_selected, selected_features
    

def lasso_feature_selection(X_train: pd.DataFrame, y_train: pd.Series, X_test: pd.DataFrame,
                            y_test: pd.Series, num_features: int, penalty: str = DEFAULT_PENALTY_LASSO,
                            solver: str = DEFAULT_SOLVER_LASSO, max_iter: int = 1000,
                            random_state: int = DEFAULT_RANDOM_STATE, verbose: int = 1,
                            lasso_params: Optional[Dict] = None) -> Tuple[pd.DataFrame, pd.DataFrame, 
                                                                          List[str]]:
    ...
    if num_features <= 0: raise ValueError("num_features must be a positive integer.")
    if num_features > X_train.shape[1]: raise ValueError("num_features cannot be greater than the total number of features.")
    if penalty not in ['l1', 'l2']: raise ValueError("penalty must be either 'l1' or 'l2'.")
    if solver not in ['liblinear', 'saga'] and penalty == 'l1': raise ValueError("For L1 penalty, solver must be 'liblinear' or 'saga'.")
    
    # Initialize LogisticRegression
    lasso_params = lasso_params or {}
    lasso = LogisticRegression(penalty=penalty, solver=solver, max_iter=max_iter,
                               random_state=random_state, **lasso_params)
    ...
    # Transform datasets
    X_train_selected = pd.DataFrame(model.transform(X_train),
                                    columns=X_train.columns[model.get_support()],
                                    index=X_train.index)
    X_test_selected = pd.DataFrame(model.transform(X_test),
                                   columns=X_train.columns[model.get_support()],
                                   index=X_test.index)
    
    # Get selected features
    selected_features = X_train.columns[model.get_support()].tolist()
    if verbose: logger.debug(f"Selected {len(selected_features)} features using Lasso")
    
    return X_train_selected, X_test_selected, selected_features
    

def shap_feature_selection(X_train: pd.DataFrame, y_train: pd.Series, X_test: pd.DataFrame,
                           y_test: pd.Series, num_features: int, threads: int,
                           iterations: int = DEFAULT_ITERATIONS_SHAP,
                           learning_rate: float = DEFAULT_LEARNING_RATE_SHAP,
                           depth: int = DEFAULT_DEPTH_SHAP, verbose: int = 1,
                           catboost_params: Optional[Dict] = None,
                           sample_size: int = 1000) -> Tuple[pd.DataFrame, pd.DataFrame, 
                                                             List[str]]:
    ...
    model = CatBoostClassifier(iterations=iterations, learning_rate=learning_rate,
                               depth=depth, thread_count=threads,
                               verbose=verbose > 1, **catboost_params)
    
    # Fit the model
    model.fit(X_train, y_train)
    
    # Explain model with SHAP values
    if verbose: logger.debug("Calculating SHAP values...")
    
    # Subsample if dataset is large
    if len(X_train) > sample_size:
        X_train_sampled = X_train.sample(n=min(sample_size, len(X_train)), 
                                         random_state=DEFAULT_RANDOM_STATE)
    else: X_train_sampled = X_train
    ...
    if verbose: logger.debug(f"Selected {len(selected_features)} features using SHAP")
...
def generate_shap_report(model, X: pd.DataFrame, K: int = 10) -> Tuple[str, pd.DataFrame]:
    ...
    for feat in top_feats:
        vals = X[feat].values
        shap_vals = sv[:, X.columns.get_loc(feat)]
        rho_tuple = spearmanr(vals, shap_vals)
        rho = rho_tuple[0]
        strength = "strong" if abs(rho) > 0.5 else "moderate" if abs(rho) > 0.3 else "weak"
        trend = "positive" if rho > 0 else "negative"
        lines.append(f" • `{feat}` shows a {strength} {trend} monotonic relationship (ρ = {rho:.2f})")
    ...
    except Exception: lines.append("Could not compute SHAP interaction values.")
    ...
    for i, feat in enumerate(top_feats):
        ...
        if mean_abs_int is not None:
            ...
            if rho_i > 0 and rho_j > 0: relation = "mutually reinforcing"
            elif rho_i < 0 and rho_j < 0: relation = "mutually diminishing"
            elif rho_i * rho_j < 0: relation = "opposing or balancing"
            else: relation = "complex or weak"

            lines.append(f" • `{feat}` interacts most with `{partner}` (mean |interaction SHAP| = {score:.3f}); relationship: {relation} (ρ_feat→SHAP = {rho_j:.2f}, ρ_partner→SHAP = {rho_i:.2f})")
    ...
    # Create DataFrame
    report_df = pd.DataFrame({'feature': features, 'mean_abs_shap': mean_abs_shap,
...
def perform_feature_selection(X_train: pd.DataFrame, y_train: pd.Series, X_test: pd.DataFrame,
                              y_test: pd.Series, feature_selection: str = DEFAULT_METHOD,
                              use_permutation_importance: bool = DEFAULT_USE_PERMUTATION_IMPORTANCE,
                              thread_count: int = DEFAULT_THREAD_COUNT, step_size: int = DEFAULT_STEP_SIZE,
                              num_features: int = DEFAULT_NUM_FEATURES, random_state: int = DEFAULT_RANDOM_STATE,
                              verbose: int = 1, feature_selection_params: Optional[Dict] = None,
                              perm_importance_scorer: Optional[Callable] = None, perm_importance_n_repeats: int = 10,
                              catboost_params: Optional[Dict] = None) -> Tuple[pd.DataFrame, pd.DataFrame, List[str]]:
    """Perform feature selection using specified method with optional 
    permutation importance."""
    # Input validation
    _validate_inputs(X_train, y_train, X_test, y_test)
    valid_methods = ['rfe', 'select_k_best', 'chi_squared', 'lasso', 'shap']
    if feature_selection not in valid_methods: raise ValueError(f"feature_selection must be one of {valid_methods}")
    if num_features <= 0 or step_size <= 0 or thread_count <= 0: raise ValueError("num_features, step_size, and thread_count must be positive integers.")
    ...
    if use_permutation_importance:
        if verbose: logger.debug("Computing permutation importances...")

        # Create default scorer if not provided
        if perm_importance_scorer is None: perm_importance_scorer = make_scorer(matthews_corrcoef)
        if not callable(perm_importance_scorer): raise TypeError("perm_importance_scorer must be a callable function")
            
        perm_model = CatBoostClassifier(iterations=500, learning_rate=0.1,
                                        depth=4, verbose=verbose > 1,
                                        thread_count=thread_count,
                                        random_state=random_state,
                                        **catboost_params)
        perm_model.fit(X_train_selected, y_train,
                       eval_set=(X_test_selected, y_test),
                       verbose=verbose > 1,
                       early_stopping_rounds=100)
        
        # Compute permutation importance
        perm_importance = permutation_importance(perm_model, X_test_selected.values,
                                                 y_test, scoring=perm_importance_scorer,
    ...
    if verbose: logger.debug(f"Selected {len(selected_features)} features")
    
    return X_train_selected, X_test_selected, selected_features
    

def train_and_evaluate(X_train: pd.DataFrame, y_train: pd.Series, X_test: pd.DataFrame,
                       y_test: pd.Series, params: Dict, output_dir: Union[str, Path],
                       plot: bool = False, verbose: int = 1, early_stopping_rounds: int = 100,
                       custom_metrics: Optional[Dict[str, Callable]] = None) -> Tuple[CatBoostClassifier, 
                                                                                      float, float, float, 
                                                                                      pd.Series]:
    ...
    if verbose: logger.debug(f"Training with parameters: {params}")
    
    # Train the model
    model.fit(X_train, y_train, eval_set=(X_test, y_test),
              plot=plot, verbose=verbose > 1,
    ...
    if verbose: logger.debug(f"Model evaluation: Accuracy={accuracy:.4f}, F1={f1:.4f}, MCC={mcc:.4f}")
